...
export type AccessRole = "hrd" | "jobseeker" | "superadmin"
...
export async function createHrdJob(
  ...
  const { error } = await supabase.from("job_vacancies").insert({
    ...
    metadata: {
      ...
      role_name: input.title,
      min_years_experience: input.minYears,
    },
  })
...
export async function updateHrdJob(
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      job_description: input.description.trim(),
      requirements: input.skills,
      role_name: input.title,
      metadata: {
        ...meta,
...
async function listCandidateRankingAnalyses(): Promise<
  CandidateRankingAnalysis[]
> {
  const supabase = getSupabaseAdmin()
  const { data, error } = await supabase
    .from("analysis_results")
  ...
  const jobseekerIds = [
    ...new Set((data ?? []).map((row) => String(row.jobseeker_id))),
  ]
  const { data: users, error: usersError } = jobseekerIds.length
    ? await supabase
        .from("users")
  ...
  return (data ?? []).map((row) => {
    const responsePayload =
      typeof row.response_payload === "object" && row.response_payload !== null
        ...
        : null
    const contact = contactByUserId.get(String(row.jobseeker_id))

    return {
      analysisId: String(row.analysis_id),
      candidateProfile: responsePayload?.candidateProfile,
      candidateEmail: contact?.email ?? null,
      candidateName: contact?.name ?? null,
