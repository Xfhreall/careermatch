...
import type { JobseekerChatbotResponse } from "@/features/jobseeker-chatbot/api-client"

const { sendMessage } = vi.hoisted(() => ({
  sendMessage: vi.fn(),
}))

vi.mock("@/features/auth/user-query", () => ({
  useUserQuery: () => ({ data: null }),
}))

vi.mock("@/features/cv-analysis/api-client", () => ({
...
vi.mock("@/features/jobseeker-chatbot/api-client", async () => {
  const actual = await vi.importActual<
    typeof import("@/features/jobseeker-chatbot/api-client")
  >("@/features/jobseeker-chatbot/api-client")
